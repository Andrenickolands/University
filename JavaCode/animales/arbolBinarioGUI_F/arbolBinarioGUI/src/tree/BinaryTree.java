...
public class BinaryTree {
    ...
    public BinaryTree(NodoPanel nodoPanel) {
    ...
    public void randomizeTree() {
        ...
        int numNodos = rand.nextInt((16 - 5) + 1) + 5;
        Nodo root = new Nodo(0, 0, 24, 1, 0, Color.GREEN);
        LinkedList<Nodo> randomTree = new LinkedList<>();
        randomTree.add(root);
        int i = 0;
        while (i < numNodos) {
            ...
            while (!successfullyAdded) {
                Nodo chosenNodo = randomTree.get(rand.nextInt(randomTree.size()));
                newNodo = new Nodo(0, 0, 24, rand.nextInt(200), chosenNodo.getLevel() + 1, Color.BLUE);
                if (rand.nextBoolean() && chosenNodo.getLeftChild() == null) {
                    chosenNodo.setLeftChild(newNodo);
                    successfullyAdded = true;
                } else if (chosenNodo.getRightChild() == null) {
                    chosenNodo.setRightChild(newNodo);
                    successfullyAdded = true;
                }
            }
            randomTree.add(newNodo);
            i++;
        }
        addNodosToPanelInOrder(root);
    ...
    public void sort() {
        ...
        BinarySearchTree sortedTree = new BinarySearchTree(new Nodo(0, 0, 24, rootVal, 0, Color.BLUE));
